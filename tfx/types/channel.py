...
class Channel(json_utils.Jsonable):
  ...
  def __init__(
    ...
    Args:
      ...
      matching_channel_name: This targets to the key of a input Channel dict
  ...
  def create_artifacts(self, count: int) -> None:
    """Creates multiple artifacts in the Channel."""
    self._artifacts = [self.type() for _ in range(count)]
