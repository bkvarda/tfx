...
class ChannelTest(tf.test.TestCase):
  ...
  def testDynamicArtifacts(self):
    channel = Channel(type=_MyType, matching_channel_name='test')
    self.assertEqual('test', channel.matching_channel_name)

    channel.create_artifacts(5)
    self.assertLen(channel._artifacts, 5)
    self.assertEqual(_MyType, channel._artifacts[0].type)
